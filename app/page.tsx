...
import { CropIcon as Corn, Sun, Users, ShoppingCart, ShoppingBag, Truck, FileText } from 'lucide-react'
...
function QuickAccessButton({ icon, label, href }: { icon: React.ReactNode; label: string; href: string }) {
  return (
    ...
    >
      <div className="text-[#2C5F2D] mb-2 transition-transform duration-200 ease-in-out group-hover:scale-110">{icon}</div>
      <span className="text-sm font-medium text-center transition-colors duration-200 ease-in-out group-hover:text-white">{label}</span>
    </Link>
  )
}
  ...
  return (
    <div className="min-h-screen bg-[#F4F1DE] text-[#5E503F]">
      <header className="bg-[#2C5F2D] text-white p-4 flex flex-col sm:flex-row justify-between items-center">
        <h1 className="text-2xl font-bold mb-4 sm:mb-0">Farmer's Dashboard</h1>
      ...
      <nav className="bg-[#F4F1DE] p-4">
        <div className="grid grid-cols-2 sm:grid-cols-7 gap-4 max-w-7xl mx-auto">
          <QuickAccessButton icon={<Corn />} label="Crop Guide" href="/crop-guide" />
          <QuickAccessButton icon={<Sun />} label="Weather Forecast" href="/weather" />
          <QuickAccessButton icon={<Users />} label="Community Forum" href="/community" />
          <QuickAccessButton icon={<ShoppingCart />} label="Marketplace" href="/marketplace" />
          <QuickAccessButton icon={<ShoppingBag />} label="Known Buyers" href="/buyers" />
          <QuickAccessButton icon={<Truck />} label="Transportation" href="/transportation" />
          <QuickAccessButton icon={<FileText />} label="Add Data" href="/add-data" />
        </div>
      </nav>
...
export default function Dashboard() {
  return (
    <RefreshProvider refreshInterval={300000}> {/* Refresh every 5 minutes */}
      <DashboardContent />
    </RefreshProvider>
  )
}
