...
async function fetchRecentPosts(): Promise<Post[]> {
  ...
  return [
    ...
    },
    {
      ...
      topic: "Coffee Prices",
      content:
        "Coffee prices are on the rise! Great news for us coffee farmers.",
      ...
      replies: [
        {
          ...
          author: "Emma Brown",
          content:
            "That's excellent! Do you know how long this trend is expected to last?",
    ...
    },
    {
      ...
      content:
        "I've implemented a drip irrigation system and it's saving so much water!",
      timestamp: "1 day ago",
      replies: [],
    },
  ];
...
export default function CommunityWidget() {
  const {
    ...
    isLoading,
    error,
  } = useDataRefresh<Post[]>(fetchRecentPosts, []);
  ...
  return (
    ...
    >
      <h2 className="text-lg font-semibold mb-4 flex items-center">
        <Users className="mr-2 text-[#2C5F2D] w-5 h-5 sm:w-6 sm:h-6" />{" "}
        Community Highlights
      </h2>
      {error ? (
        <p className="text-red-500">
          Error loading community posts. Please try again.
        </p>
      ) : (
        <>
            ...
            {recentPosts.map((post) => (
              <li key={post.id} className=" pb-2 p-2 rounded-md">
                ...
                <p className="text-xs sm:text-sm mb-1">
                  <strong>{post.topic}:</strong> {post.content}
                </p>
